...
public class StageA_StaticMatch : MonoBehaviour, ITrainingStage
{
    public string StageName => "Stage A - Static Shapes (2D Match)";
    ...
    public void OnUserResponse()
    {
        ...
        session.LogTrialLine($"StageA : left shape {leftName} - right shape {rightName}");
    ...
    private IEnumerator RunTrial()
    {
        ...
        {
            Debug.LogError("StageA_StaticMatch: boardUI or its images are not assigned.");
            yield break;
        }
        ...
        // ✅ Cue: green only when it is a MATCH
